...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        var destination = SampleRenderer.GetDestinationDirectory();
        destination.Create();
        destination.Delete(true);

        var directory = SampleRenderer.FindRepositoryDirectory();
        string basePath = Path.Combine(directory.FullName, "src/3rdParty/RPGCore.Documentation/Samples");
                    ...
                    if (method.GetParameters().Length == 0)
                    {
                        ...
                        if (attribute.Format == OutputFormat.Json)
                        {
                            if (result is string stringResult)
                            {
                                ...
                                folderName = folderName[..folderName.LastIndexOf('.')];
