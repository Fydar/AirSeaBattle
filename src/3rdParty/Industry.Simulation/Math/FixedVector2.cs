...
public readonly struct FixedVector2 : IEquatable<FixedVector2>
{
    ...
    public readonly Fixed Magnitude => FixedMath.Sqrt(SqrMagnitude);

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    public readonly Fixed SqrMagnitude => (X * X) + (Y * Y);

    public readonly FixedVector2 Normalized => Normalize(this);

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private readonly string DebuggerDisplay => string.Format(CultureInfo.InvariantCulture, "({0}, {1})", X.AsFloat, Y.AsFloat);
